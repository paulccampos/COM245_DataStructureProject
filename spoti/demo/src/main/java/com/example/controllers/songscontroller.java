...
public class songscontroller {
    ...
    private void populateFilters() {
        ...
        // Populate Genre ComboBox dynamically
    ...
    private void loadSongs() {
        ...
        if (selectedGenre != null && !"All Genres".equals(selectedGenre)) {
            songs.removeIf(song -> {
                Object genreObj = song.get("genre");
                if (genreObj instanceof List<?> genreList) {
                    // Check if the selected genre is present in the list.
                    // The 'not' (!) at the beginning is because we are using removeIf.
                    // We remove the song if its genre list does NOT contain the selected genre.
