...
public class ProfileController implements Initializable {
    ...
    public ProfileController() {
        ...
        userService = App.getUserServiceStatic();
    }
    ...
    private void onLogout() throws IOException {
        // Stop playing song and clear queue
        App app = new App();
        app.getMediaPlayerService().pause();
        app.getMediaPlayerService().clearQueue();

        // Clear current user session
        App.setCurrentUser(null);

        // Clear remembered credentials file
        try {
            java.io.File file = new java.io.File("user_credentials.dat");
            if (file.exists()) {
                file.delete();
            }
        } catch (Exception e) {
