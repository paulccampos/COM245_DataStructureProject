...
public class homecontroller {
    ...
    private VBox createPlaylistBox(Document playlist) {
        ...
        // Get top 4 songs from the playlist
        String playlistName = playlist.getString("title");
        String username = App.getCurrentUsername();
        List<Document> songs = mongoService.getSongsForPlaylist(playlistName, username);
        songs.sort((a, b) -> Integer.compare(b.getInteger("play_count", 0), a.getInteger("play_count", 0)));
        songs = songs.stream().limit(4).collect(Collectors.toList());

        // Create a 2x2 grid for the top song covers
        GridPane grid = new GridPane();
        grid.setHgap(2);
        grid.setVgap(2);
        grid.setAlignment(Pos.CENTER);

        for (int i = 0; i < Math.min(4, songs.size()); i++) {
            ImageView iv = new ImageView();
            iv.setFitHeight(55);
            iv.setFitWidth(55);
            iv.setPreserveRatio(true);
            iv.setStyle("-fx-background-radius: 5;");
            String imagePath = songs.get(i).getString("imagePath");
